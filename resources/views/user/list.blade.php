...
@section('page_header_right')
    <a href="{{url('setting/user/create')}}" class="btn btn-primary btn-fw">Add new</a>
@endsection
...
<script>
    ...
    $(document).on('change','#search', function(){
        fetch_tabledata('/setting/user');
    })

    $('.delete_btn').click(function(e){
        e.preventDefault();
        var modalConfirm = $('#modal_confirm');
        modalConfirm.find('form').attr('action',"{{url('setting/user')}}/"+$(this).data('id'));
        modalConfirm.find('#confirm_title').html('Delete user ');
        modalConfirm.find('#confirm_titlecaption').html('Are you sure delete user ');

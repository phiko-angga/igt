...
@section('page_header_right')
    <a href="{{url('master/aldeia/create')}}" class="btn btn-primary btn-fw">Add new</a>
@endsection
...
<div class="pd-20 card-box mb-30">
    ...
    <table class="table table-striped list-table">
        ...
        <tbody>
            @include('aldeia.list_pagination')
        </tbody>
    </table>
</div>
...
<script>
    ...
    $(document).on('change','#search', function(){
        fetch_tabledata('/master/aldeia');
    })

    $('.delete_btn').click(function(e){
        e.preventDefault();
        var modalConfirm = $('#modal_confirm');
        modalConfirm.find('form').attr('action',"{{url('master/aldeia')}}/"+$(this).data('id'));
        modalConfirm.find('#confirm_title').html('Delete aldeia ');
        modalConfirm.find('#confirm_titlecaption').html('Are you sure delete aldeia ');
